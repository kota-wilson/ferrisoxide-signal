...
| WRA-RQ-007 | The system shall define pass/fail criteria in a config shape. | User request | High | Criteria model covers min/max voltage and leaves room for pulse/glitch/dropout checks. | Software Architect | Partial: CLI min/max criteria implemented; config shape deferred |
