...
| WRA-TASK-080 | Record architecture diagram hierarchy rule and current-crate backfill | Documentation Engineer / Software Architect | User documentation-rule recommendation | `docs/architecture/ferrisoxide-overview.md` defines the high-level system flowchart; `docs/architecture/crate-architecture-audit.md` records the audit; `AGENTS.md` and `docs/architecture.md` define major crate criteria, focused crate-local `architecture.md` requirement, Mermaid component/data-flow diagram requirement, and no-duplicate-system-diagram rule; all 16 current crate-local architecture files exist | Documentation Gate | Complete for current crates; monitor future changes |
