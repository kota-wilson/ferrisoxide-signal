...
Complete the WRA-RQ-140 current-crate architecture backfill: one root FerrisOxide overview flowchart plus focused crate-local Mermaid architecture diagrams for every current major workspace crate.
...
WRA-RQ-140 is implemented for the current workspace in `AGENTS.md`, `docs/architecture.md`, `docs/architecture/ferrisoxide-overview.md`, `docs/architecture/crate-architecture-audit.md`, and all 16 current crate-local `architecture.md` files: the root overview file owns the high-level FerrisOxide system flowchart, while each current major workspace crate now has a focused crate-local Mermaid component/data-flow diagram. Crate diagrams cover inputs, internal processing stages, outputs, public APIs, and important error paths without duplicating the full system diagram. Future new major crates and material responsibility, public API, boundary, or data-flow changes must update the relevant diagram before merge readiness.
...
- Risk: Future major crate changes may drift from focused crate-local architecture diagrams if WRA-RQ-140 is not enforced during review.
...
- Decision: Require a system overview plus focused crate-local architecture documentation for every major workspace crate.
  Owner: Documentation Engineer / Software Architect
  Status: Implemented for all current workspace crates in WRA-RQ-140, `AGENTS.md`, `docs/architecture.md`, `docs/architecture/ferrisoxide-overview.md`, `docs/architecture/crate-architecture-audit.md`, and `crates/*/architecture.md`; new major crates and material crate responsibility, public API, boundary, or data-flow changes must include or update `crates/<crate-name>/architecture.md` with a focused Mermaid component/data-flow diagram before merge readiness.
