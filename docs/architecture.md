...
The system is a Rust Cargo workspace with a reusable core library and a small CLI. The architecture separates raw data ingestion, waveform modeling, filter transformations, criteria evaluation, and report rendering so future GUI or language bindings can reuse the core without depending on CLI concerns.
...
| `filter` | `crates/wra-core/src/filter.rs` | Filter trait, low-pass filter, and moving-average filter. |
...
CSV input
  ...
  -> Waveform model
  -> Filter chain produces derived waveform
  -> Criteria evaluator
  -> Analysis report
  -> CLI output
```
