...
| `ferrisoxide-deployment` | `crates/ferrisoxide-deployment` | Reviewable controller/RTOS deployment package manifest and qualification evidence report schemas. | Deployment package metadata, target profile metadata, mode profiles, required artifact roles, artifact validation, production/test/signal-validation mode policy validation, separated production/test config validation, checksum index wording helper, typed qualification evidence report, and report validation helpers. |
...
| `ferrisoxide-simulator` | `crates/ferrisoxide-simulator` | Deterministic virtual controller simulation over production control configs and abstract sample frames. | Simulation input frames, pure state-machine execution over `ferrisoxide-control-schema`, control-state trace evidence, output traces, fault traces, and structured simulation errors. |
| `ferrisoxide-verification-schema` | `crates/ferrisoxide-verification-schema` | Versioned test verification config schema for controller-in-the-loop qualification workflows. | Package metadata, production-control manifest link, approval metadata, sample timing, channels, timing windows, expected transitions, voltage limits, pulse widths, transient/dropout limits, stable-state requirements, evidence settings, report settings, parse helpers, and structured validation errors. |
...
| `ferrisoxide-deployment` | `crates/ferrisoxide-deployment/src/lib.rs` | Deployment package manifest schema, mode-profile schema, qualification evidence report schema, and validation helpers; no export command, runtime loader, signing, authentication, HAL, SDK, target hardware, or certification behavior. |
...
Controller deployment package flow
  ...
  -> Mode profiles separate production_control, test_verification, and signal_validation purposes
...
| Controller config parity | `crates/ferrisoxide-cli` focused test and `tests/controller_parity/` notes | Same production config, test verification config, channel map, and waveform input produce matching portable state trace and criteria evidence fields across desktop workflow and embedded-compatible borrowed-rule criteria path. |
| Qualification evidence report format | `crates/ferrisoxide-deployment` tests, `docs/qualification-evidence-report.md`, and `examples/deployment-package/heated-actuator/qualification-report.json` | Exact JSON fixture links config versions, channel map, simulation trace, criteria evidence, deployment metadata, checksum evidence, timestamp, and non-certification scope notes. |
